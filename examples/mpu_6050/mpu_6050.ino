// A lot of this code is from the Arduino webpage for the MPU-6050;
// but with many comments removed.  You can find a link to it here:
//    http://playground.arduino.cc/Main/MPU-6050
//
// Thanks given to arduino.cc user "Krodal," for writing all of the
// MPU 6050 related code.
// A lot of this code is from the Arduino webpage for the MPU-6050; but with many comments removed
...
// These are the "signals" that the C program sends to the arduino
// telling it to either sample the MPU6050, or to send it over.
#define MAKE_READING         100
#define SEND_X_ACCEL_VAL     2
#define SEND_Y_ACCEL_VAL     3
#define SEND_Z_ACCEL_VAL     4
#define SEND_X_GYRO_VAL      5
#define SEND_Y_GYRO_VAL      6
#define SEND_Z_GYRO_VAL      7
#define SEND_TEMP_VAL        8
#define SEND_X_ACCEL_REG_H   9
#define SEND_X_ACCEL_REG_L  10
#define SEND_Y_ACCEL_REG_H  11
#define SEND_Y_ACCEL_REG_L  12
#define SEND_Z_ACCEL_REG_H  13
#define SEND_Z_ACCEL_REG_L  14
#define SEND_X_GYRO_REG_H   15
#define SEND_X_GYRO_REG_L   16
#define SEND_Y_GYRO_REG_H   17
#define SEND_Y_GYRO_REG_L   18
#define SEND_Z_GYRO_REG_H   19
#define SEND_Z_GYRO_REG_L   20
#define SEND_TEMP_REG_H     21
#define SEND_TEMP_REG_L     22
...
// Program variables for us
int error;
accel_t_gyro_union mpu;
byte n;
int error_led = 8;
byte buffer[15];             // Packet size
...
int doFullReading() {
  return MPU6050_read (MPU6050_ACCEL_XOUT_H, (uint8_t *) &mpu, sizeof(mpu));
}

// Makes the "data packet," for us
void mkPacket() {
  // Error first
  buffer[0] = (byte)error;
  
  // Acceloremter second
  buffer[1] = mpu.reg.x_accel_h;
  buffer[2] = mpu.reg.x_accel_l;  
  buffer[3] = mpu.reg.y_accel_h;
  buffer[4] = mpu.reg.y_accel_l;  
  buffer[5] = mpu.reg.z_accel_h;
  buffer[6] = mpu.reg.z_accel_l;
  
  // Gyro Data third
  buffer[7] = mpu.reg.x_gyro_h;
  buffer[8] = mpu.reg.x_gyro_l;
  buffer[9] = mpu.reg.y_gyro_h;
  buffer[10] = mpu.reg.y_gyro_l;
  buffer[11] = mpu.reg.z_gyro_h;
  buffer[12] = mpu.reg.z_gyro_l;
  
  // Last; temp data
  buffer[13] = mpu.reg.t_h;
  buffer[14] = mpu.reg.t_l;
}
